...
func (s *VotingContract) RegisterVoter(ctx contractapi.TransactionContextInterface, voterID, name string) error {
...
func (s *VotingContract) RegisterElection(ctx contractapi.TransactionContextInterface, electionID, name string, candidates []string, startTime, endTime string) error {
...
func (s *VotingContract) CastVote(ctx contractapi.TransactionContextInterface, voterID, electionID, candidate string) error {
	isVoterExists, err := s.IsStateExists(ctx, voterID)
	if err != nil {
...
func (s *VotingContract) TallyVotes(ctx contractapi.TransactionContextInterface, electionID string) (map[string]int, error) {
	isElectionExists, err := s.IsStateExists(ctx, electionID)
	if err != nil {
...
func (s *VotingContract) GetFinalElectionResult(ctx contractapi.TransactionContextInterface, electionID string) (map[string]int, error) {
	isElectionExists, err := s.IsStateExists(ctx, electionID)
	if err != nil {
		return nil, fmt.Errorf("GetFinalElectionResult(): %s", err.Error())
	}

	if !isElectionExists {
		return nil, fmt.Errorf("GetFinalElectionResult(): election does not exists with electionID: %s", electionID)
	}
	...
	currentTime := time.Now()
	if currentTime.Before(election.StartTime) {
		return nil, fmt.Errorf("GetFinalElectionResult(): election with electionID: %s not started yet", electionID)
	}

	if currentTime.Before(election.EndTime) {
		return nil, fmt.Errorf("GetFinalElectionResult(): election with electionID: %s not ended yet", electionID)
	}
...
func (s *VotingContract) IsStateExists(ctx contractapi.TransactionContextInterface, key string) (bool, error) {
